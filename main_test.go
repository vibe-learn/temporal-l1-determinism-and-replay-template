...
func TestTaskQueueDefault(t *testing.T) {
	if os.Getenv("TEMPORAL_TASK_QUEUE") == "" && TaskQueue() != "lesson-l1_determinism_and_replay-tq" {
		t.Errorf("TaskQueue() default = %q, want lesson-l1_determinism_and_replay-tq", TaskQueue())
	}
}
...
func TestOrderWorkflow(t *testing.T) {
	...
	env.OnActivity(ProcessOrder, mock.Anything).Return("ok", nil)
	env.OnActivity(AuditOrder, mock.Anything).Return("ok", nil)
...
func TestIntegration(t *testing.T) {
	...
	// поведения урока «Детерминизм и replay».
}
